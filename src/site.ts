export const SITE = {
  name: "Roberto Salas",
  title: "Roberto Salas — Software engineer",
  description:
    "Notes on software, security, and AI by Roberto Salas, a software engineer based in Mexico.",
  email: "hi@beto.page",
  // Second line of the home title; one is picked at random on each load.
  // Simple HTML is allowed here (the strings are ours, not user input).
  taglines: [
    "I <s>google</s> AI things professionally.",
    "Stack Overflow raised me.",
    "I read the docs afterwards, like everyone.",
    "I have 47 tabs open and no answers.",
    "It works. I don't know why either.",
    "I fixed it by deleting the test.",
    "I write code until it stops complaining.",
    "My best feature is Ctrl+Z.",
    "I clap when the build passes.",
    "I name variables 'asdas' and mean it.",
    "Senior enough to break it, junior enough to panic.",
    "My git history is a diary of mistakes.",
    "I'm the reason we have code review.",
    "I nod in meetings I don't understand.",
  ],
  // Icon paths from Tabler Icons (MIT), 24x24 outline.
  links: [
    {
      label: "Resume (PDF)",
      href: "/cv.pdf",
      newTab: true,
      icon: [
        "M14 3v4a1 1 0 0 0 1 1h4",
        "M17 21h-10a2 2 0 0 1 -2 -2v-14a2 2 0 0 1 2 -2h7l5 5v11a2 2 0 0 1 -2 2",
        "M11 12.5a1.5 1.5 0 0 0 -3 0v3a1.5 1.5 0 0 0 3 0",
        "M13 11l1.5 6l1.5 -6",
      ],
    },
    {
      label: "GitHub",
      href: "https://github.com/rsalas6",
      icon: [
        "M9 19c-4.3 1.4 -4.3 -2.5 -6 -3m12 5v-3.5c0 -1 .1 -1.4 -.5 -2c2.8 -.3 5.5 -1.4 5.5 -6a4.6 4.6 0 0 0 -1.3 -3.2a4.2 4.2 0 0 0 -.1 -3.2s-1.1 -.3 -3.5 1.3a12.3 12.3 0 0 0 -6.2 0c-2.4 -1.6 -3.5 -1.3 -3.5 -1.3a4.2 4.2 0 0 0 -.1 3.2a4.6 4.6 0 0 0 -1.3 3.2c0 4.6 2.7 5.7 5.5 6c-.6 .6 -.6 1.2 -.5 2v3.5",
      ],
    },
    {
      label: "LinkedIn",
      href: "https://linkedin.com/in/rsalas6/",
      icon: [
        "M8 11v5",
        "M8 8v.01",
        "M12 16v-5",
        "M16 16v-3a2 2 0 1 0 -4 0",
        "M3 7a4 4 0 0 1 4 -4h10a4 4 0 0 1 4 4v10a4 4 0 0 1 -4 4h-10a4 4 0 0 1 -4 -4l0 -10",
      ],
    },
    {
      label: "X",
      href: "https://x.com/_rsalas6",
      icon: [
        "M4 4l11.733 16h4.267l-11.733 -16l-4.267 0",
        "M4 20l6.768 -6.768m2.46 -2.46l6.772 -6.772",
      ],
    },
  ],
};
